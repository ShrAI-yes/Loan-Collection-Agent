...
class SuperAgent:
    ...
    def extract_user_data(self, phone):
        """
        Extracts user data from the database based on the phone number.
        """
        user_data = self.file.fetch_user(phone_no=phone)
        if not user_data:
            raise ValueError(f"No user data found for phone number: {phone}")
        return user_data
